(function(){
  ...
  const applyThemeLang = async ()=>{
    ...
    // üstte:
    const baseURL = document.querySelector('base')?.href || (new URL('.', location.href)).href;

    // sözlük fetch’leri:
    const common   = await loadJSON(new URL(`data/i18n/common.${lang}.json`, baseURL).href);
    const pageDict = await loadJSON(new URL(`data/i18n/${page}.${lang}.json`, baseURL).href);
    ...
    // PDF/Link satırlarını (i18n + href) hydrate et
  function hydrateLinkOrPdf(){
    // alt dizin güvenliği: <base> varsa onu baz al
    const baseURL = document.querySelector('base')?.href || (new URL('.', location.href)).href;

    document.querySelectorAll('.i18n-link-or-pdf').forEach(row=>{
      const aOpen = row.querySelector('a.open');
      const aDl   = row.querySelector('a.download');

      // data-* kaynakları
      const openHref = row.dataset.openHref;
      const dlHref   = row.dataset.downloadHref || openHref;

      if (aOpen && openHref){
        // new URL ile normalize → hem mutlak hem göreli çalışır
        aOpen.href = new URL(openHref, baseURL).href;
        aOpen.target = '_blank';
        aOpen.rel = 'noopener';
      }
      if (aDl && dlHref){
        aDl.href = new URL(dlHref, baseURL).href;
        aDl.setAttribute('download', ''); // her dilde indir tetiklesin
      }
    });
  }
...
(async function(){
  ...
  let me = posts.find(p => path.endsWith(p.url) || p.url.endsWith(path) || p.url === path);
   if (!me) {
...
if (tocBox){
  const tocTitle = tocBox.querySelector('h3');
  // Başlangıç: mobilde kapalı, tablette/açıkta açık kalsın
  const setTocMode = ()=>{
    if (window.innerWidth <= 1024){
      tocBox.classList.remove('open');   // kapalı başlasın
      tocTitle.setAttribute('role','button');
      tocTitle.setAttribute('tabindex','0');
    } else {
      tocBox.classList.add('open');      // masaüstünde hep açık
      tocTitle.removeAttribute('role');
      tocTitle.removeAttribute('tabindex');
    }
  };
  setTocMode();
  window.addEventListener('resize', setTocMode);
  ...
  tocTitle?.addEventListener('keydown', e=>{
